...
import useSideCategory from "@/hooks/useSideCategory";
...
export default function Sidebar() {
  ...
  // const {categories} = useSideCategory();
    const { categories } = useSideCategory();

  const BUSINESS_TYPES_MERGED: BusinessItem[] = [
    ...Array.from(new Set(categories.map(c => c.business_type))).map(name => ({
      name,
      count: categories.filter(c => c.business_type === name).length,
      slug: makeSlug(name)
    })),
  ...
  const INDUSTRIES_MERGED: BusinessItem[] = [
    ...Array.from(new Set(categories.map(c => c.industry))).map(name => ({
      name,
      count: categories.filter(c => c.industry === name).length,
      slug: makeSlug(name)
    })),
...
const BUSINESS_TYPES_DYNAMIC: BusinessItem[] =
  Array.from(new Set(categories.map(c => c.business_type))).map(name => ({
    name,
    count: categories.filter(c => c.business_type === name).length
  }));

const INDUSTRIES_DYNAMIC: BusinessItem[] =
  Array.from(new Set(categories.map(c => c.industry))).map(name => ({
    name,
    count: categories.filter(c => c.industry === name).length
  }));

const DISTRICTS_DYNAMIC =
  Array.from(new Set(categories.map(c => c.district)));
          ...
          <div className="mb-6 border-b border-green-600 pb-4">
            ...
            <nav>
              <div className="space-y-1">
                ...
                {BUSINESS_TYPES_MERGED.map((type) => (
                  <FilterItem 
                    key={type.name}
                    name={type.name}
                    count={type.count} // Passed count property
          ...
          <div className="mb-6 border-b border-green-600 pb-4">
            ...
            >
              <nav className="space-y-1 py-2">
                ...
                {INDUSTRIES_MERGED.map((category) => (
                  <FilterItem 
                    key={category.name}
                    name={category.name}
                    count={category.count}
...
//   Brush,
//   Newspaper,
//   Home, // FIXED: Tally changed to Tally5
//   BookOpen,
//   Ship,
//   Sprout,
//   Lamp,
...
//   };
//   // ---------- Dynamic data from API ----------
//   const BUSINESS_TYPES_DYNAMIC: BusinessItem[] = Array.from(
//     new Set(categories.map((c) => c.business_type)),
//   ).map((name) => ({
//     name,
//     count: categories.filter((c) => c.business_type === name).length,
//   }));

//   const INDUSTRIES_DYNAMIC: BusinessItem[] = Array.from(
//     new Set(categories.map((c) => c.industry)),
//   ).map((name) => ({
//     name,
//     count: categories.filter((c) => c.industry === name).length,
//   }));

//   const DISTRICTS_DYNAMIC = Array.from(
//     new Set(categories.map((c) => c.district)),
//   );

//   const toggleBusinessType = (name: string) =>
//     toggleSelection(setSelectedBusinessTypes, name);
//   const toggleIndustry = (name: string) =>
//     toggleSelection(setSelectedIndustries, name);
...
//               <div className="space-y-1">
//                 {/* {BUSINESS_TYPES.map((type) => ( */}
//                 {BUSINESS_TYPES_DYNAMIC.map((type) => (
//                   <FilterItem
//                     key={type.name}
//                     name={type.name}
//                     count={type.count} // Passed count property
...
//               <nav className="space-y-1 py-2">
//                 {/* {ALL_CATEGORIES.map((category) => ( */}
//                 {INDUSTRIES_DYNAMIC.map((category) => (
//                   <FilterItem
//                     key={category.name}
//                     name={category.name}
//                     count={category.count}
...
//             {DISTRICTS_DYNAMIC.map((district) => (
//               <button
//                 key={district}
//                 className="w-full text-left pl-5 py-1.5 text-xs rounded-md hover:bg-green-700/70 transition"
//               >

...
function CategoryHoverItem({ category }: { category: Category }) {
    ...
    const handleToggle = (e: React.MouseEvent<HTMLAnchorElement>) => {
        if (hasSubCategories) { e.preventDefault(); setIsOpen((prev) => !prev); }
    };
    return (
        <div className="last:border-b-0 relative">
            <a href={category.href} onClick={handleToggle} className={`flex items-center justify-between px-4 py-2 transition text-white hover:bg-white/10 ${isOpen ? "bg-white/20 font-semibold" : ""}`}>
                <span><span className="truncate max-w-[calc(100%-4rem)] text-lg">{category.name}</span></span>
                {hasSubCategories && (<ChevronRightIcon className={`h-5 w-5 transition-transform duration-300 ${isOpen ? "rotate-90" : ""}`}/>)}
            </a>
            {hasSubCategories && (
                <div ref={contentRef} style={{maxHeight: isOpen ? contentRef.current?.scrollHeight : 0, transform: `scaleX(${isOpen ? 1 : 0})`,}}
                    ...
                    <nav className="flex flex-col space-y-1 p-2">
                        {category.subCategories.map((subCat) => (<a key={subCat.name} href={subCat.href} className="block px-3 py-1 text-base text-white/80 hover:bg-white/10 rounded transition">- {subCat.name}</a>))}
                    </nav>
                </div>
            )}
        </div>
...
export default function App() {
  ...
  return (
    <div className="bg-gray-50 w-full font-inter">
      ...
      {/* Hero Section */}
      <section className="flex flex-col lg:flex-row h-auto lg:h-[70vh]">
        {/* Left Sidebar */}
        <div
        ...
        <div className="w-full lg:flex-grow h-[25vh] lg:h-full relative p-7">

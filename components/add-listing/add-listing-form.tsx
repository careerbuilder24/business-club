...
export default function AddListingForm() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, type: "logo" | "cover" | "gallery") => {
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8 space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold mb-2 text-foreground">Listing Name *</label>
            <input
              type="text"
              name="listingName"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-foreground">Company Address *</label>
            <input
              type="text"
              name="address"
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-foreground">Facebook URL</label>
            <input
              type="url"
              name="facebook"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Logo */}
          <div>
            <label className="block text-sm font-semibold mb-2 text-foreground">Company Logo</label>
            <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer">
              ...
              <label htmlFor="logo-input" className="cursor-pointer">
                ...
                {images.logo && <p className="text-xs text-primary mt-2">✓ {images.logo.name}</p>}
              </label>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer">
              ...
              <label htmlFor="cover-input" className="cursor-pointer">
                ...
                {images.cover && <p className="text-xs text-primary mt-2">✓ {images.cover.name}</p>}
              </label>
            </div>
          </div>
        </div>
        ...
        {/* Gallery Images */}
        <div className="mt-6">
          <label className="block text-sm font-semibold mb-2 text-foreground">Gallery Images</label>
          <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer">
            <input
              type="file"
              accept="image/*"
              ...
              {images.gallery.map((file, idx) => (
                <div key={idx} className="relative group">
                  <div className="bg-muted rounded-lg p-2 aspect-square flex items-center justify-center">
                    <span className="text-xs text-muted-foreground text-center truncate">{file.name}</span>
                  </div>
                  <button
                    type="button"
      ...
      {/* Labels */}
      <div>
        <h2 className="text-2xl font-bold mb-6 text-foreground">Listing Labels</h2>
        ...
        <div className="flex flex-wrap gap-2">
          {formData.labels.map((label) => (
            <div key={label} className="bg-primary/10 text-primary px-3 py-1 rounded-full flex items-center gap-2">
      ...
      {/* Description */}
      <div>
        <h2 className="text-2xl font-bold mb-6 text-foreground">Description</h2>
